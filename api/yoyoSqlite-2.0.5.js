 ...
 * 作者:Yoyo
 * 功能:链式数据库操作
 * 版本:v2.0.5(全预处理版本) 安全性提升|防注入
 * 说明:使用本库,请保留自带打印说明! 否则请勿使用,我不喜欢不尊重别人劳动成果的人
 */
logger.warn('Yoyo: 感谢使用 链式数据库v2.0.5 封装模块!仓库地址(https://gitee.com/Y_oyo/yoyo-mcbe-lite-xloader-item/tree/master/sql)');
class sqlite {
    ...
    insert(data) {
        ...
        var result = sqlite.execute(sql, allValue);
        return result;
    ...
    update(data) {
        ...
        var result = sqlite.execute(sql, tolArr);
        return result;
    }
    ...
    delete() {
        ...
        var result = sqlite.execute(sql, this.#sqloptions.where.bindValue);
        return result;
    }
    ...
    find() {
        ...
        var result = sqlite.query(sql, this.#sqloptions.where.bindValue);
        return result.get(0);
    }
    ...
    */
    ...
    generateSql(type) {
        ...
        if (sqlite.isShowSql) log('sqlite: ' + resultSql);//打印输出
        return resultSql;
    }
    ...
    */
    static execute(sql, bind) {
        ...
        return DBStmt.affectedRows < 1 ? 0 : DBStmt.affectedRows;
    }
